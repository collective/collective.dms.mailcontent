...
im_irn_conditions = SimpleVocabulary(
    [SimpleTerm(u"show", title=_("Always show")), SimpleTerm(u"hide", title=_("Always hide"))]
)

om_irn_conditions = SimpleVocabulary(
    [
        SimpleTerm(u"show", title=_("Always show")),
...
class IDmsMailConfig(model.Schema):
    ...
    model.fieldset(
        "incomingmail", label=_(u"Incoming mail"), fields=["incomingmail_number", "incomingmail_talexpression"]
    )
    ...
    outgoingmail_edit_irn = schema.Choice(
        title=_(u"Internal reference number edition field"), vocabulary=om_irn_conditions, default="show"
    )
