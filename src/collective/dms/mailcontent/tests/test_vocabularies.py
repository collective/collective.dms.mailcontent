...
class TestVocabularies(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.im1 = api.content.create(
            container=self.portal, type="dmsincomingmail", id="im1", title=u"I mail", treating_groups=["Administrators"]
        )
        createContentInContainer(self.im1, "dmsmainfile", title=u"D001", file=NamedBlobFile(filename=u"scanned.pdf"))
        createContentInContainer(
            self.im1, "dmsappendixfile", title=u"A001", file=NamedBlobFile(filename=u"appendix.odt")
        )
        ...
        self.od1 = createContentInContainer(
            self.om1, "dmsmainfile", title=u"D011", file=NamedBlobFile(filename=u"generated.odt")
        )
        createContentInContainer(
            self.om1, "dmsappendixfile", title=u"A011", file=NamedBlobFile(filename=u"appendix1.pdf")
        )
        ...
        self.od2 = createContentInContainer(
            self.om2, "dmsmainfile", title=u"D021", file=NamedBlobFile(filename=u"generated.odt")
        )
        createContentInContainer(
            self.om2, "dmsappendixfile", title=u"A021", file=NamedBlobFile(filename=u"appendix2.pdf")
        )
    ...
    def test_EmailAttachmentsVocabulary(self):
        ...
        self.assertListEqual(
            ...
            [u"(dmsappendixfile) => A011  (\xab appendix1.pdf \xbb)", u"(dmsmainfile) => D011"],
        )
        self.od3 = createContentInContainer(
            self.om1, "dmsmainfile", title=u"D012", file=NamedBlobFile(filename=u"mailing.odt")
        )
        ...
        self.od4 = createContentInContainer(
            self.om2, "dmsmainfile", title=u"D022", file=NamedBlobFile(filename=u"mailing.odt")
        )
